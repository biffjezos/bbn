...
4. **CodeQL alerts** — run `git fetch origin dev --quiet && git show origin/dev:.claude/codeql-alerts.md 2>/dev/null` to read the latest alerts file directly from `origin/dev` (regardless of current session branch). A GitHub Actions workflow writes this file every Wednesday at 11:15 after the scan completes. If the file was written since the most recent Wednesday and has not been reviewed this session, report the alerts to the owner and note that **CodeQL alerts take priority over tickets**. Do not fetch live from the GitHub API on every session — only the workflow does that. If the file is missing or outdated (older than last Wednesday), note it briefly and move on.
