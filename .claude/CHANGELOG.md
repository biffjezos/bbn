...
- 2026-03-31 — CHANGE: Removed all specs-related content from CLAUDE.md (Spec Workflow section, Update specs commit step, Update spec files wrap-up step, .claude/specs/ Persistent Files entry) and deleted .claude/specs/ directory entirely per owner instruction.
- 2026-03-31 — REFLECTION: [Scope creep / False confidence] Session 1 introduced a guard in placeSelfMarker() blocking guest self markers entirely, and a refreshRadius() call in onLogout() that read the stale registered-user token — both introduced as "fixes" but broke the intended behaviour. Root cause: changed working code without reading the pre-T28 commit behaviour first. Rule: before changing map icon or auth timing logic, read the git history (last commit before T-28 migration) to confirm intended behaviour. Do not add guards or calls without verifying the exact timing of token/state variables.
- 2026-03-31 — REFLECTION: [Under-reading] Incomplete Edit of placeSelfMarker() left a dangling `else {` block — the function was structurally broken and required two correction passes. Root cause: code inserted into the middle of a function without reading the full body first. Fix: always read the complete function before a structural edit; old_string must include enough surrounding context to confirm placement is correct.
- 2026-03-31 — REFLECTION: [Under-reading] Incomplete Edit of placeSelfMarker() left a dangling `else {` block — the function was structurally broken and required two correction passes. Root cause: code inserted into the middle of a function without reading the full body first. Fix: always read the complete function before a structural edit; old_string must include enough surrounding context to confirm placement is correct.
- 2026-03-31 — REFLECTION: [Under-reading] map.js onLogout() was committed in the previous session without verifying it against the spec. The spec said "clear all markers, favour lines, meeting control, and radius" but selfMarker, selfCircle, viewRadius, and lastSex were not cleared — causing stale sex-aware icon and wrong radius after logout. Rule: after implementing any function described by a spec, verify the code matches every bullet in expected_behaviour before committing.
- 2026-03-31 — REFLECTION: [Pattern blindness] ES module top-level code (window.__authReady?.then, window.Auth hook in prefs.js) silently failed because these globals are set inside initApp() which runs on DOMContentLoaded — after module evaluation. No error is thrown; callbacks are just discarded. Rule: never access window.__authReady or window.Auth at ES module top level. All post-auth work must go through exported functions called from boomboom.js at the correct point in initApp().
- 2026-03-31 — REFLECTION: [Pattern blindness] Multiple modules (lock.js, favourites.js, messages.js, admin.js) had broken or missing initialization because they depended on window.Auth/BbmPrefs being set, DOMContentLoaded timing, or auth resolving — but these contracts were never written down. Root cause: specs were not required for these modules. Rule already in place ("any module with security-relevant behaviour requires a spec") but was not enforced for lock.js at creation time. Outcome: four new security specs created to prevent future breakage.
- 2026-03-31 — CHANGE: CLAUDE.md Spec Workflow — corrected prior entry: lock.js, prefs.js DO require specs (they have security-relevant and initialization-order contracts); the exemption for "thin UI event handlers" applies only to truly single-purpose DOM wrappers with no inter-module dependencies.
- 2026-03-30 — REFLECTION: [False confidence / Under-reading] Wired login/register submit listener AFTER `await window.__authReady`. During the guest auth warm-up window, no listener was attached and browser fell back to native GET form submission; email and password string appeared in URL and server logs (SEC-1.15). Root cause: did not reason about the async gap between DOM-ready and listener attachment. Rule: any security-sensitive form listener must be wired synchronously at DOM-ready, never after an async wait. `method="POST"` and `onsubmit="return false"` must be on every form with a submit button at creation time, not added later as hardening.
- 2026-03-30 — CHANGE: Tightened Spec Workflow in CLAUDE.md — replaced named module inclusion/exclusion lists with the general rule: "any module with inter-module contracts, async lifecycles, security-relevant behaviour, or initialization-order dependencies requires a spec; when in doubt, write the spec." Specific named lists removed to prevent future omissions for unlisted but qualifying modules.
