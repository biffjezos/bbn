...
- 2026-03-25 — REFLECTION: **Scope creep + False confidence** — Previous session changed Rust CodeQL `build-mode` from `none` to `autobuild` without being asked to, and without verifying that Rust actually supports autobuild in CodeQL (it does not — only `none` is valid). The change was framed internally as "fixing low analysis quality" but was unsolicited and broke the weekly scan. Rule added to CLAUDE.md: never change any CI/CD or workflow configuration beyond what was explicitly requested. If a CI quality issue is observed, log it as an audit item and ask the owner — do not self-authorize a fix.
...
- 2026-03-25 — REFLECTION: **Incomplete commit** — ran `cargo build` to verify the workspace fix, but forgot to stage the resulting `Cargo.lock` update. The stop hook caught it. Rule: after running any `cargo build` or `cargo check` as part of verification, always `git status` before committing to catch lock file or generated file changes.
...
- 2026-03-25 — REFLECTION: **Under-reading** — Wrote a workflow that commits directly to `dev` without first checking whether `dev` is a protected branch. The push failed with GH006. Rule: before writing any workflow that pushes to a branch, confirm whether that branch has protection rules. Ask the owner if unknown. — Asked owner to paste GITHUB_TOKEN into chat to enable live CodeQL API calls at session start. Neither necessary (the workflow + git approach already covered the use case without any token) nor viable (settings.local.json is gitignored and doesn't survive session resets). Rule: before asking for any credential, verify (1) it is actually required, and (2) there is a durable place to store it that survives session boundaries. If neither is true, redesign the approach first.
- 2026-03-25 — REFLECTION: **Over-explanation** — first draft of pre-session CodeQL step was 5 sentences explaining the scheduled agent, the "don't fetch live" rationale, and fallback behaviour. Owner asked "do you really need this paragraph?" Answer was no. Rule: CLAUDE.md instructions should state what to do, not why the design was chosen. Keep instruction prose to the minimum that makes the action unambiguous.

- 2026-03-25 — REFLECTION: **Scope creep + over-explanation** — Owner asked "how do I make sure the opaque client works?" (an informational question). Instead of answering directly, Claude read 6+ files, explored the full Rust crate, the server-side auth code, and the generated WASM JS glue, then started writing a test script. The answer was 6 sentences. Rule added: treat questions as questions first — if no ticket is referenced and no code change is implied, answer in prose before reaching for any tool.
...
- 2026-03-24 — REFLECTION: [Wrong target] When inserting a new struct before an existing `#[derive(Clone)] struct AppState`, the derive attribute stayed in place and applied to the new struct instead of AppState, causing a conflicting-impl compile error. Fix: always include the derive attribute in the old_string when editing a struct definition so the replacement keeps it correctly positioned.
