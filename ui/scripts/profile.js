...
// renderMyProfile() · renderPublicProfile()
// ============================================================
...
export async function renderMyProfile() {
  ...
  const current = window.Auth.getProfile();
    ...
    <div class="bbm-profile-form">
      ...
      <div class="mb-4">
        ...
        <input
          ...
          class="form-control"
          id="editNickname"
          value="${escHtml(current.nickname || '')}"
          minlength="2"
          maxlength="32"
          placeholder="Display name"
        />
        <div class="mt-1" style="font-size:0.78rem; color: var(--bbm-text-faint)">
          Shown to nearby users. Not unique.
        </div>
      </div>
  ...
  document.getElementById('changePasswordBtn').addEventListener('click', () => {
    ...
    section.className = 'mt-4 pt-4 bbm-profile-form';
    ...
    section.innerHTML = `
      ...
      <div class="mb-3">
        ...
        <input type="password" class="form-control" id="currentPw" autocomplete="current-password" />
      </div>
      <div class="mb-3">
        <label class="form-label" for="newPw">New Password</label>
        <input type="password" class="form-control" id="newPw" autocomplete="new-password" placeholder="At least 8 characters" />
      </div>
      <div class="mb-4">
        <label class="form-label" for="confirmPw">Confirm New Password</label>
        <input type="password" class="form-control" id="confirmPw" autocomplete="new-password" />
      </div>
      <button class="btn btn-bbm-primary" id="savePwBtn">
...
export async function renderPublicProfile() {
  ...
  try {
    ...
    const cls     = sexAvatarClass(profile.sex);
    ...
    const guestBlock = !isRegistered ? `
      <div class="mt-5 pt-4" style="border-top:1px solid var(--bbm-border)">
        ...
        <div class="d-flex gap-3 flex-wrap">
          ...
          <button class="btn btn-bbm-ghost" data-bs-toggle="modal" data-bs-target="#loginModal">
            Log In
          </button>
        </div>
      </div>` : '';

    const actionBlock = isRegistered ? `
      <div class="d-flex gap-3 flex-wrap mt-4">
        <a href="${threadHref}" class="btn btn-bbm-pink">
          <i class="bi bi-chat-dots me-2"></i>Message
        </a>
        <button
          class="btn ${isFav ? 'btn-bbm-outline-pink' : 'btn-bbm-ghost'}"
          id="favToggleBtn"
          data-userid="${escHtml(userId)}"
          data-fav="${isFav}"
        >
          <i class="bi bi-star${isFav ? '-fill text-pink' : ''} me-2"></i>
    ...
    page.innerHTML = `
      ...
      <div class="bbm-pub-profile-hero">
        <div class="container-fluid px-4 px-md-5">
          ...
          <p class="bbm-pub-meta">
            ${profile.age ? escHtml(String(profile.age)) + ' · ' : ''}${sexLabel(profile.sex)}
          </p>
          ${actionBlock}
          ${guestBlock}
        </div>
    ...
    page.innerHTML = `
      <div class="container-fluid px-4 px-md-5 py-5">
        ...
        <a href="/" class="btn btn-bbm-ghost mt-2">
          <i class="bi bi-arrow-left me-2"></i>Back to Map
        </a>
      </div>`;
  }
}
